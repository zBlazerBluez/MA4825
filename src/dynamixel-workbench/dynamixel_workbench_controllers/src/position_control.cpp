...
bool PositionControl::input2ValCB(dynamixel_workbench_msgs::Input2Val::Request &req,
               dynamixel_workbench_msgs::Input2Val::Response &res)
{
	int32_t radian_value = dxl_wb_->convertRadian2Value(req.id, req.radian);
	int32_t speed_value = dxl_wb_->convertVelocity2Value(req.id, req.speed);

	res.radian_result = radian_value;
	res.speed_result = speed_value;
}
