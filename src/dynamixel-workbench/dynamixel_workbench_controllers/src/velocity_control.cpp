...
{
  static int32_t goal_velocity[6] = {0, 0, 0, 0, 0, 0};

  //goal_velocity[0] = dxl_wb_->convertVelocity2Value(dxl_id_[0], req.vel1);
  //bool ret1 = dxl_wb_->goalSpeed(dxl_id_[0], goal_velocity[0]);

  goal_velocity[1] = dxl_wb_->convertVelocity2Value(dxl_id_[0], req.vel2);
  bool ret2 = dxl_wb_->goalSpeed(dxl_id_[0], goal_velocity[1]);

  goal_velocity[2] = dxl_wb_->convertVelocity2Value(dxl_id_[1], req.vel3);
  bool ret3 = dxl_wb_->goalSpeed(dxl_id_[1], goal_velocity[2]);

  //goal_velocity[3] = dxl_wb_->convertVelocity2Value(dxl_id_[1], req.vel4);
  //bool ret4 = dxl_wb_->goalSpeed(dxl_id_[1], goal_velocity[3]);

  //goal_velocity[4] = dxl_wb_->convertVelocity2Value(dxl_id_[4], req.vel5);
  //bool ret5 = dxl_wb_->goalSpeed(dxl_id_[4], goal_velocity[4]);

  //goal_velocity[5] = dxl_wb_->convertVelocity2Value(dxl_id_[5], req.vel6);
  //bool ret6 = dxl_wb_->goalSpeed(dxl_id_[5], goal_velocity[5]);

  //printf("goal_velocity: %d, %d\n", goal_velocity[0], goal_velocity[1]);
